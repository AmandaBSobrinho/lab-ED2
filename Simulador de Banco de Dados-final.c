...
Arvore *inserir(Arvore *raiz, Aluno *aluno, Aluno *novo){
	...
	if(!raiz){// se não foi criada a árvore, cria e inicializa sua raiz
	...
	//verificar se raiz é um nó não folha (ver se há algum apontamento)
	for(i = 0; i < 4 && !encontrou; i++)
		if(raiz->filhos[i] != NULL)
			encontrou = 1;
	
	if(encontrou){// for um nó não folha
