...
void print_leaves( node * root ) {
	...
	while (true) {
		for (i = 0; i < c->num_keys; i++) {
			if (verbose_output)
				printf("%d ", (unsigned long*)c->pointers[i]);
			printf("%d ", c->keys[i]);
		}
		if (verbose_output)
...
node * delete_entry( node * root, node * n, int key, void * pointer ) {
	...
	neighbor =(node*) neighbor_index == -1 ?(node*) n->parent->pointers[1] : 
...
Arvore *encontrarFolha(Arvore **alunos, int endereco){
	...
	if(!aux)
		return NULL;
	int i;
	while(!aux->is_leaf){// procurando um nó ate que ele sea uma folha
		i = 0;//se não é o contador reseta
		while(i < aux->num_keys){//para um nó, procurar o filho para "descer" de nível
			if(endereco >= aux->keys[i])
				i++;
			else
				break;
		}
		aux = aux->pointers[i];//"descendo de nível"
	}
	return aux;//se chegou até aqui encontrou uma folha
}
...
void inserirArvore(Arvore **alunos, int endereco){
	...
	//REMOVER DEPOIS******************************************
	*alunos = insert(*alunos, endereco, endereco);
...
void excluirAluno(Arvore **arvoreAlunos, FILE *arq){
	...
	scanf("%d", &numero);
	*arvoreAlunos = Delete(*arvoreAlunos, numero);
}

void imprimirLista(Arvore **arvoreAlunos){
	print_leaves(*arvoreAlunos);
}

void imprimirArvore(Arvore **arvoreAlunos){
	print_tree(*arvoreAlunos);
}
